...
export class CitiesComponent implements OnInit {
  ...
  findCity(name) {
    this._cityService.getCity(name).subscribe(response => {
  ...
  getCurrentWeather(key, name) {
    this._cityService.getCurrent(key).subscribe(response => {
      this.city = response;
  ...
  addCity(name, ciudad) {
    ...
    this.newCity = new Ciudad(
      name,
      ciudad[0].Temperature.Metric.Value,
      ciudad[0].WeatherText,
      ciudad[0].WeatherIcon,
      ciudad[0].IsDayTime
    );
